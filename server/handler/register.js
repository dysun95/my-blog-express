...
let createUser = require('../db/create/user')
let updateUser = require('../db/update/user')
      ...
      if (result && result.name === user.name) {
        ...
        createUser(user).then(result => {
          if (result.result && result.result.ok === 1) { // 用户信息插入成功
            ...
            updateUser({"_id": userNew._id}, {"puid": userNew._id.toString()}).then(result => { // 使用mongodb的_id作为唯一puid,更新用户信息
